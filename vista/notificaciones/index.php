...
<script src="https://pure-ravine-13656.herokuapp.com/socket.io.js"></script>

<script>
  let idAreaSocket = <?php echo $idArea; ?>;
  console.log("id="+idAreaSocket);
 
  var socket = io.connect('https://pure-ravine-13656.herokuapp.com/');
  
  socket.on('nuevosNumeros', function(msg) {
	let isSelected = false;  
	for(let contador of msg.numAsuntos){
		if(contador.area == idAreaSocket && isSelected == false ){
			console.log("total1="+contador.total);
			console.log("area1="+contador.area);
			$("#numActualizado").html(contador.total);
			isSelected = true;
		
		}else if(isSelected == false )
		{
			$("#numActualizado").html('0');
		}
	...
	for(let contador of msg.numAsuntosEnviados ){
		if(contador.area == idAreaSocket && isSelected == false ){
			console.log("total2="+contador.total);
			console.log("area2="+contador.area);
			console.log("cadena");
			$("#numActualizadoEnviado").html(contador.total);
			isSelected = true;
		}else if(isSelected == false )
		{
			
			$("#numActualizadoEnviado").html('0');
		}
	...
	for(let contador of msg.numAsuntosSinR ){
		if(contador.area == idAreaSocket && isSelected == false ){
			console.log("total2="+contador.total);
			console.log("area2="+contador.area);
			console.log("cadena");
			$("#numActualizadoSinRespuesta").html(contador.total);
			isSelected = true;
		}else if(isSelected == false )
		{
			
			$("#numActualizadoSinRespuesta").html('0');
		}
	...
	for(let contador of msg.numAsuntosSinREnviados ){
		if(contador.area == idAreaSocket && isSelected == false ){
			console.log("total2="+contador.total);
			console.log("area2="+contador.area);
			console.log("cadena");
			$("#numActualizadoSinRespuestaEnviados").html(contador.total);
			isSelected = true;
		}else if(isSelected == false )
		{
			
			$("#numActualizadoSinRespuestaEnviados").html('0');
		}
	...
	/*
	console.log(msg.idArea);
	
	if(msg.idArea == idAreaSocket){
		$("#numActualizadoSinRespuesta").html(msg.numAsuntosSinR[0].total);
		$("#numActualizadoSinRespuestaEnviados").html(msg.numAsuntosSinREnviados[0].total);
	}*/

  });
  /*
  socket.on('nuevosNumerosEnviados', function(msg) {
	for(let contador of msg){
		if(contador.area == idAreaSocket){
			console.log("total="+contador.total);
			console.log("area="+contador.area);
			$("#numActualizadoEnviado").html(contador.total);
			return;
		}else 
		{
...
function abreAsuntos(tipo,estatus)
{
	...
	$(padre).find('iframe#scorm_object').attr('src','apps/Asuntos/index.php?ac=1&anio='+anio+'&idArea='+idArea+'&idUsuario='+idUsuario+'&opcion='+tipo+'&tipo=0&estatus='+estatus+'&idEje=0&idAreaU='+idAreaUsuario);
