<?php
	...
	$asuntos = $act -> mostrarDetalleRecibidos($idDestino,$idOrigen,$idEje,$tipo);
	$cadenaTabla = "";
	$i = 1;
	$color = "";
	$ultimaAct = "";
	...
	foreach($asuntos as $asunto)
	{
		...
		if($asunto->getEstatus() == 1){
			...
			$iconResp = "<i onclick='responder(".$asunto->getId().")' style='cursor:pointer;' class='fas fa-user-edit'></i>";
			
			//$resp = "<b onclick='respondeAsunto".$i."();'>R5</b>";
		}else if($asunto->getEstatus() == 2){
			$color = "background-color: #efd707;";
			$iconResp = "<i onclick='responder(".$asunto->getId().")'  style='cursor:pointer;' class='fas fa-user-edit'></i>";
			$icon = "<i onclick='conversacion(".$asunto->getId().",".$idDestino.",".$idOrigen.")' style='cursor:pointer;' class='far fa-comments'></i>";
			//$resp = "<b onclick='respondeAsunto".$i."();'>R</b>";
		}else{
	...
	$cadenaTabla .= "<tr><th style='width: 25px;".$color."' scope='row'>".$i."<br><br>".$icon."<br><br>".$iconResp."<br>".$resp."</th><td>Enviado por : ".$asunto->getNombreArea()." (".$asunto->getNombreUsuario().")<br><div style='font-size: 8.5px;'>".$asunto->getNombreEje()."<br>".$ultimaAct."</div> <div class='respuesta'><b style='text-decoration: underline #607D8B;'>".$asunto->getDescripcion()."</b><br>".$asunto->getRespuesta()." <div class='fecha'>".date('Y-m-d H:i', strtotime($asunto->getFecha()))."</div></div> <div class='divResponde' id='responde".$i."'></div></td><td>".$onclick."</td></tr>"; 
...
function responder(idConversacion) 
{
	...
	location.replace("/sie/vista/apps/Asuntos/index.php?ind=1&ac=1&idUsuario="+idUsuario+"&idArea="+idArea+"&anio="+anio+"&tipo=4&opcion=recibido&idEje=0&idAreaU="+idAreaUsuario+"&idConversacion="+idConversacion); 
}
