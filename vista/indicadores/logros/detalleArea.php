<?php
	...
	$logrosArea = $actLogro -> mostrarDetallePorAreaArea($idEje,$idArea);
	...
	$logros = $actLogro -> mostrarDetalleArea($idEje,$idArea);
	$cadena ="";
	$area ="";
	foreach($logros as $logro)
	{
		$area = $logro->getNombreArea();
		...
		$cadena .="<div class='panel panel-default font'><div class='panel-heading' style='padding: 3px;'>".$logro->getIdEje().".".$logro->getOrden()." ".$logro->getNombreActividad()." (".$logro->getNombreArea().")<br>".$logro->getNombrePersona()."<i onclick='editar(".$logro->getId().")' style='float: right;cursor:pointer' class='fas fa-edit'></i></div></div>";
...
<body>
<div style="text-align:center;"><b><?php echo $area; ?></b></div>
...
function editar(idLogro)
{
	var idLogro = idLogro;
	
	location.replace("/sie/vista/apps/Logros/Alta_logro.php?accion=editar&tipo_de_resumen=3&id="+idLogro+"&act_met=1&origen=10"); 
