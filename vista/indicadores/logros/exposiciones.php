<?php
	...
	foreach($exposiciones as $expo){
		...
		$exposLogros = $actLogro  -> mostrarTotalLogrosActividadesExpoNoEditados($expo->getId());
		$totalExpoLogro = $exposLogros -> getTotal();
		
		$exposLogrosT = $actLogro  -> mostrarTotalLogrosActividadesExpoTerminados($expo->getId());
		$totalExpoLogroTer = $exposLogrosT -> getTotal();
		
		$exposLogrosE = $actLogro  -> mostrarTotalLogrosActividadesExpoEditados($expo->getId());
		$totalExpoLogroE = $exposLogrosE -> getTotal();
		
		$exposLogrosEnt = $actLogro  -> mostrarTotalLogrosActividadesExpoEntregable($expo->getId());
		$totalExpoLogroEnt = $exposLogrosEnt -> getTotal();
		
		$exposLogrostotal = $actLogro  -> mostrarTotalLogrosActividadesExpo($expo->getId());
		...
		$cadenaTabla .= "<tr><td>(".$expo->getAnio().") ".$expo->getNombreExposicion()." <p class='avanceExpo'>".number_format($percent, 0, '.', '')."% avance<br><meter min='0' max='100' low='20' high='60' optimum='100'  value='".number_format($percent, 0, '.', '')."'></meter></p></td><td onclick='abreSubModal(7,\"Logros de Exposiciones temporales\",\" de actividades sin llenar (".$expo->getNombreExposicion().")\",".$totalExpoLogro.",1,".$expo->getId().");' style='cursor:pointer;color:red'>".$totalExpoLogro."</td><td onclick='abreSubModalTerminado(7,\"Logros de Exposiciones temporales\",\" de actividades terminados (".$expo->getNombreExposicion().")\",".$totalExpoLogroTer.",1,".$expo->getId().");'  style='cursor:pointer;color:#107c10'>".$totalExpoLogroTer."</td><td onclick='abreSubModalProceso(7,\"Logros de Exposiciones temporales\",\" de actividades en proceso (".$expo->getNombreExposicion().")\",".$totalExpoLogroE.",1,".$expo->getId().");' style='cursor:pointer;color:#ffb900'>".$totalExpoLogroE."</td><td onclick='abreModalEntregableExpo(7,1,".$expo->getId().",\" (".$expo->getNombreExposicion().")\")' style='cursor:pointer;color:#2196F3' >".$totalExpoLogroEnt."</td></tr>";
