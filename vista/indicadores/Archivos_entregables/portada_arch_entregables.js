            ...
            (data, status) => {
                if (status == "success") {
                    ...
                    $.each(obj, function (i, item) {
                      ...
                      rows+='</li>';
                    $("#total_").append(' : '+item.total);
                    });
                    rows+='</ul>';

                    $("#arch_ent_total").append(rows);
                }
            });
    },
          ...
          {accion:"por_eje",fecha:fecha,eje_filtro:eje_filtro,area_filtro:area_filtro,exposicion:exposicion},
            (data, status) => {
                if (status == "success") {
                    ...
                    rows+='</ul>';
                    $("#eje_arch_ent").append(' : '+total);
                    $("#arch_ent_eje").append(rows);
                }
            });
    },
    archivos_por_area:(fecha,eje_filtro,area_filtro,exposicion)=>{
    let obj;
    $("#origen_arch_ent").html("");
    $("#arch_ent_origen").html("");
        $.post("./Conexiones_arch_ent.php",
          {accion:"por_area",fecha:fecha,eje_filtro:eje_filtro,area_filtro:area_filtro,exposicion:exposicion},
            (data, status) => {
                if (status == "success") {
                    ...
                    rows+='</ul>';
                    $("#origen_arch_ent").append(' : '+total);
                    $("#arch_ent_origen").append(rows);
                }
            });
    },
        ...
        {accion:"por_anio",fecha:fecha,eje_filtro:eje_filtro,area_filtro:area_filtro,exposicion:exposicion},
            (data, status) => {
                if (status == "success") {
                    ...
                    rows+='</ul>';
                    $("#anio_arch_ent").append(' : '+total);
                    $("#arch_ent_anio").append(rows);
                }
            });
    },
          ...
          {accion:"por_expo",fecha:fecha,eje_filtro:eje_filtro,area_filtro:area_filtro,exposicion:exposicion},
            (data, status) => {
                if (status == "success") {
                    ...
                    rows+='</ul>';
                    $("#expo_arch_ent").append(' : '+total);
                    $("#arch_ent_expo").append(rows);
                }
            });
    },
