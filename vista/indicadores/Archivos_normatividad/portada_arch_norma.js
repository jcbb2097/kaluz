...
let portada={
    ...
    ObtenerListadoanios:()=>{
    let obj;
        $.post("./Conexiones_arch_norma.php",
        {accion:"anios"},
            (data, status) => {
                if (status == "success") {
    ...
    let obj;
    $("#total_arch_norma").html("");
    $("#arch_norma_total").html("");
        $.post("./Conexiones_arch_norma.php",
            ...
            (data, status) => {
                if (status == "success") {
                    ...
                    $.each(obj, function (i, item) {
                      ...
                      rows+='</li>';
                    $("#total_arch_norma").append(' : '+item.total);
                    });
                    rows+='</ul>';

                    $("#arch_norma_total").append(rows);
                }
            });
    },
          ...
          {accion:"por_eje",fecha:fecha,eje_filtro:eje_filtro,area_filtro:area_filtro,exposicion:exposicion},
            (data, status) => {
                if (status == "success") {
                    ...
                    rows+='</ul>';
                    $("#eje_arch_norma").append(' : '+total);
                    $("#arch_norma_eje").append(rows);
                }
            });
    },
    archivos_por_area:(fecha,eje_filtro,area_filtro,exposicion)=>{
    let obj;
    $("#origen_arch_norma").html("");
    $("#arch_norma_origen").html("");
        $.post("./Conexiones_arch_norma.php",
          {accion:"por_area",fecha:fecha,eje_filtro:eje_filtro,area_filtro:area_filtro,exposicion:exposicion},
            (data, status) => {
                if (status == "success") {
                    ...
                    rows+='</ul>';
                    $("#origen_arch_norma").append(' : '+total);
                    $("#arch_norma_origen").append(rows);
                }
            });
    },
    archivos_por_anio:(fecha,eje_filtro,area_filtro,exposicion)=>{
    let obj;
    $("#anio_arch_norma").html("");
    $("#arch_norma_anio").html("");
        $.post("./Conexiones_arch_norma.php",
        {accion:"por_anio",fecha:fecha,eje_filtro:eje_filtro,area_filtro:area_filtro,exposicion:exposicion},
            (data, status) => {
                if (status == "success") {
                    ...
                    rows+='</ul>';
                    $("#anio_arch_norma").append(' : '+total);
                    $("#arch_norma_anio").append(rows);
                }
            });
    },
    archivos_por_expo:(fecha,eje_filtro,area_filtro,exposicion)=>{
    let obj;
    $("#expo_arch_norma").html("");
    $("#arch_norma_expo").html("");
        $.post("./Conexiones_arch_norma.php",
          {accion:"por_expo",fecha:fecha,eje_filtro:eje_filtro,area_filtro:area_filtro,exposicion:exposicion},
            (data, status) => {
                if (status == "success") {
                    ...
                    rows+='</ul>';
                    $("#expo_arch_norma").append(' : '+total);
                    $("#arch_norma_expo").append(rows);
                }
            });
    },
