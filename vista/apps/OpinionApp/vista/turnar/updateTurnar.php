...
/*Detalle*/
$act2 = new OpinionController();
$opinionDet = $act2 -> mostrarDetalleOpinionTurnada($idOpinion);


$nombreEje = $opinionDet -> getNombreEje();
$nombrePersonaResponsable = $opinionDet -> getNombrePersonaResponsable();
$nombreApPatResponde = $opinionDet -> getApPatResponde();
$nombreApMatResponde = $opinionDet -> getApMatResponde();
$nombreArea = $opinionDet -> getNombreArea();

$nombreCategoria = $opinionDet -> getNombreCategoria();
$nombreSubcategoria = $opinionDet -> getNombreSubcategoria();
$nombreActividadGlobal = $opinionDet -> getNombreActividadGlobal();
$nombreActividadGeneral = $opinionDet -> getNombreActividadGeneral();
$nombreCheck = $opinionDet -> getNombreCheck();
$nombreSubcheck = $opinionDet -> getNombreSubcheck();

$idEje = $opinionDet -> getIdEje();
$idArea = $opinionDet -> getIdArea();
$idPersonaResponsable = $opinionDet -> getIdPersonaResponsable();
$idCategoriaGenerico = $opinionDet -> getIdCategoria();
if(isset($idCategoriaGenerico)){
	$idCategoria = $idCategoriaGenerico;
}else{
	$idCategoria = 0;
}

$idSubcategoriaGenerico = $opinionDet -> getIdSubcategoria();
if(isset($idSubcategoriaGenerico)){
	$idSubcategoria = $idSubcategoriaGenerico;
}else{
	$idSubcategoria = 0;
}

$idActividadGlobalGenerico = $opinionDet -> getIdActividadGlobal();
if(isset($idActividadGlobalGenerico)){
	$idActividadGlobal = $idActividadGlobalGenerico;
}else{
	$idActividadGlobal = 0;
}

$idActividadGeneralGenerico = $opinionDet -> getIdActividadGeneral();
if(isset($idActividadGeneralGenerico)){
	$idActividadGeneral = $idActividadGeneralGenerico;
}else{
	$idActividadGeneral = 0;
}

$idCheck = $opinionDet -> getIdCheck();
$idSubcheck = $opinionDet -> getIdSubcheck();
...
foreach($ejes as $eje)
{
	if($idEje == $eje->getIdEje())
	{
		$selectEje = " selected ";
	}else{
		$selectEje = "";
	}
	$cadenanEjes .= "<option ".$selectEje." value=".$eje->getIdEje().">".$eje->getOrden().".- ".$eje->getNombre()."</option>";
}
...
foreach($areas as $area)
{
	...
	$cadenanAreas .= "<option ".$selectArea." value=".$area->getIdArea().">".$area->getNombre()."</option>";
}
...
<form style='overflow-x: hidden;' id ="frmturnaUpdate" action="" class="formName">
	<div class='row'>
...
$(function()
{
	var idPersona = <?php echo $idPersonaResponsable; ?>;
	var idArea = <?php echo $idArea; ?>;
	$.post("turnar/actualiza/personaUpdate.php",{idArea:idArea,idPersona:idPersona}, function( data ) 
	{
	...
	$.post("turnar/actualiza/categoriaUpdate.php",{idEje:idEje,anio:anio,idCategoria:idCategoria}, function( data ) 
	{
		$(".actualizaCategoria").html('');
		$(".actualizaCategoria").html(data);
	});	
	
	var idSubcategoria = <?php echo $idSubcategoria; ?>;
	$.post("turnar/actualiza/subcategoriaUpdate.php",{idCategoria:idCategoria,idSubcategoria:idSubcategoria}, function( data ) 
	{
		$(".actualizaSubcategoria").html('');
		$(".actualizaSubcategoria").html(data);
	});

	var idActividadGlobal = <?php echo $idActividadGlobal; ?>;
	var anioNumber = <?php echo $anio; ?>;
	$.post("turnar/actualiza/actividadGlobalUpdate.php",{idSubcategoria:idSubcategoria,idEje:idEje,anio:anioNumber,idActividadGlobal:idActividadGlobal}, function( data ) 
	{
		$(".actualizaActGlobal").html('');
		$(".actualizaActGlobal").html(data);
	});	

	var idActividadGeneral = <?php echo $idActividadGeneral; ?>;
	$.post("turnar/actualiza/actividadGeneralUpdate.php",{idActividadGlobal:idActividadGlobal,idEje:idEje,anio:anioNumber,idActividadGeneral:idActividadGeneral}, function( data ) 
	{
