...
if (isset($_GET['areaF']) && $_GET['areaF'] != "") {
    $areaF = $_GET['areaF'];
    //  echo "1";
} else {
...
foreach ($ejes as $eje) {
    $actividad = $Planeacion->Vista_entregables_ejeArea($eje, 1, $Year[0], $Year[1], $areaF, $Id_area);
    $meta = $Planeacion->Vista_entregables_ejeArea($eje, 2, $Year[0], $Year[1], $areaF, $Id_area);
...
$totalAv = $totalAv; //+= ($totalCAvanceAC + $totalCAvanceMT) / ($totalEje + $totalEjeAc);
...
<div class="well2 wr">
    ...
    <div class="filtroSelect" style="position: absolute;top: 40px;left: 265px;">
        <select style="width: 124px;border-radius: 0px;font-size: 9px;font-family: 'Muli-Regular';color: black;" name="verTipo" id="verTipo">
            ...
            <option selected value="2">Área</option>
            <option value="3">Persona</option>
        </select>
    </div>
    <div style="position: absolute;top: 43px;left: 400px;" class="titlefiltro">Área: &nbsp;&nbsp; <b></b></div>
    <div class="filtroSelectPersona" style="position: absolute;top: 40px;left: 452px;">
        <select style="width: 124px;border-radius: 0px;font-size: 9px;font-family: 'Muli-Regular';color: black;" name="areaF" id="areaF">
            <option value="0">Seleccione una opción</option>
            <?php
...
<body>
    ...
    ?>
        <div style="top: <?php echo $colorEjes[$i] ?>px;left: 1px;width: 400px; height: 54px; border: 1px solid white; background-color: #9d47ad78;" class="aplE">
            <div class="col-md-6 col-sm-6 col-xs-6">
               ...
               <?php 
                if($i != "6"){
                ?> 
               <a class="entregable control-label" onclick="ejeColor(<?php echo $eje ?> ,'<?php echo $colorEjes[$i]; ?>' );muestraDetalle(<?php echo $eje ?> ,1,<?php echo $Year[0] ?>,'PlaneacionArea')">
               <?php }else { ?>   
                <a class="entregable control-label" onclick="ejeColor(<?php echo $eje ?> ,'<?php echo $colorEjes[$i]; ?>' );muestraDetalle(<?php echo $eje ?> ,1,<?php echo $Year[0] ?>,'PlaneacionArea')">
                <?php } ?>
        ...
        <div style="top: <?php echo $colorEjes[$i] ?>px;left: 402px;width: 400px; height: 54px;border: 1px solid white; background-color: #82a0d79e;" class="aplE">
            <div class="col-md-6 col-sm-6 col-xs-6">
                <a class="entregable control-label" onclick="ejeColor(<?php echo $eje ?> ,'<?php echo $colorEjes[$i]; ?>' );muestraDetalle(<?php echo $eje ?> ,2,<?php echo $Year[0] ?>,'PlaneacionArea')">
...
<script>
    ...
    $('#verTipo').on('change', function() {
        ...
        var Tipo_perfil = $('#Tipo_perfil').val();
        if (tipoFiltro == "2") {
            url = "IndexTipoArea.php?nombreUsuario=" + nombreUsuario + "&idUsuario=" + Id_usuario + "&perfil=" + Perfil + "&tipoPerfil=" + Tipo_perfil + "&periodo=" + Periodo + "&areaF=" + areaF;
        }else if (tipoFiltro == "3") {
            url = "IndexTipoPersona.php?nombreUsuario=" + nombreUsuario + "&idUsuario=" + Id_usuario + "&perfil=" + Perfil + "&tipoPerfil=" + Tipo_perfil + "&periodo=" + Periodo + "&areaF=" + areaF;
        }else {
            url = "Index.php?nombreUsuario=" + nombreUsuario + "&idUsuario=" + Id_usuario + "&perfil=" + Perfil + "&tipoPerfil=" + Tipo_perfil + "&periodo=" + Periodo;
        }
    ...
    $('#areaF').on('change', function() {      
        var Persona = $('#areaF').val();
        var Periodo = $('#anio').val();
        var Id_usuario = $('#Id_usuario').val();
        var nombreUsuario = $('#nombreUsuario').val();
        var Perfil = $('#Perfil').val();
        var Tipo_perfil = $('#Tipo_perfil').val();
        url = "IndexTipoArea.php?nombreUsuario=" + nombreUsuario + "&idUsuario=" + Id_usuario + "&perfil=" + Perfil + "&tipoPerfil=" + Tipo_perfil + "&periodo=" + Periodo + "&areaF=" + Persona;   
        $(location).attr('href', url);
       // alert(Persona);
    });
    ...
    function muestraDetalle(idEje, tipo, periodo, url) {
        ...
        var areaFiltro = $("#areaF option:selected").val();

        let ejes = ['Eje 1 Estrategias de seguridad', 'Eje 2 Plan estratégico', 'Eje 3 Infraestructura', 'Eje 4 Gestion administrativa', 'Eje 5 Autogestión de recursos', 'Eje 6 Exposición permanente', 'Eje 7 Exposiciones temporales', 'Eje 8 Bellas artes para todos', 'Eje 9 Difusión e imagen', 'Eje 10 Publicaciones', 'Eje 11 Estrategia digital'];
        url = url + ".php?IdEje=" + idEje + "&tipo=" + tipo + "&Periodo=" + periodo + "&ano=" + ano + "&nombreeje=" + ejes[idEje - 1] + "&Id_usuario=" + Id_usuario + "&nombreUsuario=" + nombreUsuario + "&Perfil=" + Perfil + "&tipoPerfil=" + Tipo_perfil + "&area=" + areaFiltro;
        $(location).attr('href', url);
    }
</script>
