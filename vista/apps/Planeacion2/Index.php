...
foreach ($ejes as $eje) {
    $actividad = $Planeacion->Vista_entregables_eje($eje, 1, $Year[0], $Year[1], $persona, $Id_area);
    $meta = $Planeacion->Vista_entregables_eje($eje, 2, $Year[0], $Year[1], $persona, $Id_area);
...
<div class="well2 wr">
    ...
    <div class="filtroSelect" style="position: absolute;top: 40px;left: 265px;">
        <select style="width: 124px;border-radius: 0px;font-size: 9px;font-family: 'Muli-Regular';color: black;" name="verTipo" id="verTipo">
            ...
            <option value="4">Número de entregables</option>
        </select>
    </div>
    ...
    ?>
        <div style="top: <?php echo $colorEjes[$i] ?>px;left: 1px;width: 400px; height: 54px; border: 1px solid white; background-color: #9d47ad78;" class="aplE">
            <div class="col-md-6 col-sm-6 col-xs-6">
                ...
                <?php //}else { ?>   
                   <!-- <a class="entregable control-label" onclick="ejeColor(<?php echo $eje ?> ,'<?php echo $colorEjes[$i]; ?>' );">  -->
                <?php //} ?>
        ...
        <div style="top: <?php echo $colorEjes[$i] ?>px;left: 402px;width: 400px; height: 54px;border: 1px solid white; background-color: #82a0d79e;" class="aplE">
            <div class="col-md-6 col-sm-6 col-xs-6">
               ...
               // if($i != "6"){
                ?> 
                   <a class="entregable control-label" onclick="ejeColor(<?php echo $eje ?> ,'<?php echo $colorEjes[$i]; ?>' );muestraDetalle(<?php echo $eje ?> ,2,<?php echo $Year[0] ?>,'Planeacion_avance_acme')">
                <?php //}else { ?>   
                   <!-- <a class="entregable control-label" onclick="ejeColor(<?php echo $eje ?> ,'<?php echo $colorEjes[$i]; ?>' );"> -->
                <?php // } ?>
...
/* funcion para el cambio de anio */
    ...
    $('#verTipo').on('change', function() {
        ...
        if(verTipo == "2"){
            ...
            var Perfil = $('#Perfil').val();
            var Tipo_perfil = $('#Tipo_perfil').val();
            url = "IndexTipoArea.php?nombreUsuario=" + nombreUsuario + "&idUsuario=" + Id_usuario + "&perfil=" + Perfil + "&tipoPerfil=" + Tipo_perfil + "&periodo=" + Periodo;
            $(location).attr('href', url);
        }
        if(verTipo == "3"){
            var Periodo = $('#anio').val();
            ...
            var Perfil = $('#Perfil').val();
            var Tipo_perfil = $('#Tipo_perfil').val();
            url = "IndexTipoPersona.php?nombreUsuario=" + nombreUsuario + "&idUsuario=" + Id_usuario + "&perfil=" + Perfil + "&tipoPerfil=" + Tipo_perfil + "&periodo=" + Periodo;
            $(location).attr('href', url);
        }
        if(verTipo == "4"){
            ...
            var Perfil = $('#Perfil').val();
            var Tipo_perfil = $('#Tipo_perfil').val();
            url = "PlaneacionEntregables.php?periodo=" + Periodo + "&ano=" + ano;
            $(location).attr('href', url);
        }
    });
...
/* funcion para ver por tipo la planeacion */
    function muestraDetalle(idEje, tipo, periodo, url) {
        ...
        url = url + ".php?IdEje=" + idEje + "&tipo=" + tipo + "&Periodo=" + periodo + "&ano=" + ano + "&nombreeje=" + ejes[idEje - 1] + "&Id_usuario=" + Id_usuario + "&nombreUsuario=" + nombreUsuario + "&Perfil=" + Perfil + "&tipoPerfil=" + Tipo_perfil;
        $(location).attr('href', url);
    }
</script>
