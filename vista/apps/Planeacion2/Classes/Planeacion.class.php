...
class Planeacion
{
    ...
    public function Vista_entregables_eje($Eje, $Tipo, $Id_periodo, $Periodo)
    {
        ...
        $total_avance = 0;
        $totalEntregable = 0;
        $ids = $this->get_categorias_eje($Eje, $Tipo, $Periodo); /*aqui modifique $Periodo*/
    ...
    public function Vista_entregables_ejePersona($Eje, $Tipo, $Id_periodo, $Periodo, $personaF)
    {
        $entregables = 0;
        $totales = array();
        $Avance = 0;
        ...
        for ($i = 0; $i < count($ids); $i++) {
            $idcategoria = $this->get_categorias($ids[$i], $Tipo, $Periodo);
            $Resultados = $this->Entregables_categoriaPersona($idcategoria, $Tipo, $Id_periodo, $Periodo, $personaF);
            $entregables = $entregables += $Resultados[0];
            $Avance = $Avance + $Resultados[2];
            $totalEntregable = $totalEntregable += $Resultados[1];
            $porcentaje = $porcentaje += $Resultados[2];

           // echo "entregablesa : " . $Resultados[0];
        }
        if($entregables == 0){
          $total_avance = 0;
        }else{
          $total_avance = $Avance / $entregables;
        }
    //    echo "id : " . count($ids);
     //   echo "ave : " . $Avance;
    //
    //   echo "1 : " . $entregables;
    //   echo "2 : " . $total_avance;
    //   echo "3 : " . $totalEntregable;
        array_push($totales, $entregables, $total_avance, $totalEntregable);
        return $totales;
    }

    public function Vista_entregables_ejeArea($Eje, $Tipo, $Id_periodo, $Periodo, $personaF)
    {
        $entregables = 0;
        $totales = array();
        $Avance = 0;
    ...
    /*----------------------------------------------------------------------------------------------------*/
    //id periodo a period
    public function Subcategorias_planeacion($Idcate, $Anio, $ACME)
    {
        $avance = 0;
    ...
    WHERE
        c.idCategoriaPadre = $Idcate AND p.Id_Periodo=$Anio AND ca.Visible<>0 AND ca.ACME=$ACME";
    ...
    public function Entregables_categoria($Id_categoria, $Tipo, $Id_Periodo, $Periodo)
    {
        ...
        FROM
            ...
            INNER JOIN c_checkList ch ON ch.IdCheckList = cha.IdCheckList
            INNER JOIN c_actividad a on a.IdActividad=cha.IdActividad
            left JOIN k_actividad_categoria ca on ca.IdActividad= cha.IdActividad AND ca.IdCategoria = cha.IdCategoria AND ca.IdPeriodo = cha.Id_Periodo
            INNER JOIN k_categoriasdeejes_anios aa on aa.idCategoria=cha.IdCategoria
        WHERE
         cha.Visible = 1 AND cha.Id_Periodo = $Id_Periodo  AND ((ch.Nivel = 2 ) OR ( ch.Nivel = 1 AND ch.Tipo = 1 )OR(ch.Nivel=3))
            AND cha.IdCategoria IN ($Id_categoria) AND (aa.ACME=$Tipo AND aa.Visible=1 AND aa.Anio=$Periodo)
            AND a.IdTipoActividad=$Tipo  AND ca.Activo = 1 ";
    ...
    public function Entregables_categoriaPersona($Id_categoria, $Tipo, $Id_Periodo, $Periodo, $personaF)
    {
        $resultados = array();
        $Total_entregables = 0;
        $Total_entregables_c = 0;
        ...
        if($personaF != ""){
           ...
           $consulta = "SELECT
            COUNT(cha.IdCheckList) entregables,(SELECT SUM(cha.Avance) Avance FROM
            k_checklist_actividad cha
            INNER JOIN c_checkList ch ON ch.IdCheckList = cha.IdCheckList
            INNER JOIN c_actividad a on a.IdActividad=cha.IdActividad
            INNER JOIN k_categoriasdeejes_anios aa on aa.idCategoria=cha.IdCategoria
            WHERE
            cha.IdEncargado = $personaF and cha.Visible = 1 AND cha.Id_Periodo = $Id_Periodo  AND ((ch.Nivel = 2 ) OR ( ch.Nivel = 1 AND ch.Tipo = 1 )OR(ch.Nivel=3))
            AND cha.IdCategoria IN ($Id_categoria) AND (aa.ACME=$Tipo AND aa.Visible=1 AND aa.Anio=$Periodo)
            AND a.IdTipoActividad=$Tipo  )as avance,
            (SELECT COUNT(cha.IdCheckList) entregables FROM
            k_checklist_actividad cha
            INNER JOIN c_checkList ch ON ch.IdCheckList = cha.IdCheckList
            INNER JOIN c_actividad a on a.IdActividad=cha.IdActividad
            INNER JOIN k_categoriasdeejes_anios aa on aa.idCategoria=cha.IdCategoria
        WHERE
        cha.IdEncargado = $personaF and  cha.Visible = 1  AND cha.Id_Periodo = $Id_Periodo
            AND cha.IdCategoria IN ($Id_categoria ) AND (aa.ACME=$Tipo AND aa.Visible=1 AND aa.Anio=$Periodo)
            AND a.IdTipoActividad=$Tipo  AND ((ch.Nivel = 2 ) OR ( ch.Nivel = 1 AND ch.Tipo = 1 )OR(ch.Nivel=3))  )as entregables_com
        FROM
            k_checklist_actividad cha
            INNER JOIN c_checkList ch ON ch.IdCheckList = cha.IdCheckList
            INNER JOIN c_actividad a on a.IdActividad=cha.IdActividad
        ...
        $Total_entregables = $row2['entregables'];
       //  echo "porcentaje : " . $row2['avance'];
            $Total_entregables_c =  $row2['entregables_com'];
          //  $Total_entregables_c = $row2['avancePor'];
            if ($row2['avance'] > 0) {
                $Avance = $row2['avance'];
            }

            array_push($resultados, $Total_entregables, $Total_entregables_c, $Avance);
        }
