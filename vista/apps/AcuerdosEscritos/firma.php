...
$nombreUsuario = "SinUsr";
    //echo '<input type="text" id="tipoPerfil" name="tipoPerfil" value="' . $_GET['tipoPerfil'] . '"/>';
    //echo '<input type="text" id="idUsuario" name="idUsuario" value="' . $_GET['idUsuario'] . '"/>';
...
{   if ($_GET['areainvitada']!="0") {$AreaInvitada ="".$_GET['areainvitada'];} //Si el parametro es diferente de 0 se busca el valor
    else { $AreaInvitada="Sin valor"; } //Si el parametro es igual a 0 se buscan los NULOS
}
...
//echo $NombreTipoPerfil;
$url = "./Alta_acuerdo.php?accion=editar&id=$IdAcuerdo&tipoPerfil=$TipoPerfil&nombreUsuario=SinUsr&idUsuario=$idUsuario";

/*$consultanombre = "SELECT Id_Area,Nombre FROM c_area WHERE Id_Area=$AreaInvitada";
$resulvanombre = $catalogo->obtenerLista($consultanombre);
    while ($row = mysqli_fetch_array($resulvanombre)) {
           if ($NombreTipoPerfil == $row['Id_Area']) {
...
<!-- Formulario que recoge los datos y los enviara al servidor -->
 <form id="form" action="./savedraw.php" method="post">
    ...
    <input type="hidden" name="areainvitada" value="<?php echo $AreaInvitada; ?>" id="areainvitada">
    <br>
...
function nopuedefirmar() {
 Swal.fire({
  icon: 'error',
  title: 'No puedes firmar esta área',
  showConfirmButton: false,
  timer: 2000
}).then(function() {
    <?php
...
<?php 
//No puede firmar si no pertenece a la área convocada
$consultanombre = "SELECT Id_Area,Nombre FROM c_area WHERE Id_Area=$AreaInvitada";
$resulvanombre = $catalogo->obtenerLista($consultanombre);
    while ($row = mysqli_fetch_array($resulvanombre)) {
           if ($NombreTipoPerfil == $row['Id_Area']) {
