...
echo"<li role=\"presentation\" class=\"active\" ><a href=\"#\" onclick=\"cambiarContenidos('#contenidos', 'Menu_mis_indicadores.php');\" >Mis Indicadores</a></li>";
}
...
<div class="container-fluid">
    <div class="row">
        <div class="col-md-12 col-sm-12 col-xs-12">
            <legend>Indicadores de mi Interes</legend>
            <div class="row">
                <div class="form-group form-group-sm">
                    <label class="col-md-2 col-sm-2 col-xs-2  control-label" for="inputState">Indicador</label>
                    <div class="col-md-4 col-sm-4 col-xs-4">
                        <select id="Indicador" class="form-control" name="Indicador" onchange="Filtroindicador();"  >
                            <?php
                            $consulta = "SELECT
                                pp.id_indicador,
                                cc.Descripcion
                            FROM
                                k_indicadores AS cc
                            INNER JOIN k_indicador_usuario AS pp ON pp.id_indicador = cc.IdIndicador
                            WHERE
                                pp.id_usuario = $UsuarioPermiso
                                ORDER BY
                            cc.Descripcion ASC";
                            ...
                            while ($row = mysqli_fetch_array($resultado)) {
                                $s = '';
                                if ($row['id_indicador'] == $Interes) {
                                    $s = 'selected="selected"';
                                }
                                echo '<option value="' . $row['id_indicador'] . '" ' . $s . '>' . $row['Descripcion'] . '</option>';
                            }
                            ?>   
