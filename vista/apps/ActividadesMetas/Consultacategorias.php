...
{   if ($_GET['Eje']!="0") {$Eje =$_GET['Eje'];
    $consultacategoria = "SELECT ce.idCategoria,ce.descCategoria FROM c_categoriasdeejes ce INNER JOIN c_periodo p on p.Id_Periodo=ce.anio WHERE ce.nivelCategoria=1 AND p.Id_Periodo=9 AND ce.idEje=$Eje ORDER BY ce.orden ";
...
{   if ($_GET['EjeGlobal']!="0") {$EjeGlobal =$_GET['EjeGlobal'];
    $consultaperiodo7 = "SELECT a.IdActividad, CONCAT(a.Numeracion,a.Nombre) actividad FROM c_actividad a WHERE a.IdEje=$EjeGlobal AND a.IdNivelActividad=1 AND IdTipoActividad=$ActividadMeta and a.Periodo=9 ORDER BY a.Orden";
...
{   if ($_GET['Categoria1']!="0") {$Categoria1 =$_GET['Categoria1'];
    $consultaperiodo7 = "SELECT a.IdActividad, CONCAT(a.Numeracion,a.Nombre) actividad FROM c_actividad a WHERE a.IdEje=$EjeGlobal1 AND a.IdNivelActividad=1 AND IdTipoActividad=$ActividadMeta1 and a.Periodo=9 AND a.Idcategoria=$Categoria1 ORDER BY a.Orden";
...
{   if ($_GET['EjeEntregableGlobal']!="0") {$EjeEntregableGlobal=$_GET['EjeEntregableGlobal'];
    $consultaperiodo11 = "SELECT IdEntregable,Nombre FROM c_entregable 
    WHERE idActividad=$EjeEntregableGlobal
    order BY Nombre asc";
...
{   if ($_GET['EjeEntregableGeneral']!="0") {$EjeEntregableGeneral=$_GET['EjeEntregableGeneral'];
    $consultaperiodo11 = "SELECT IdEntregable,Nombre FROM c_entregable 
    WHERE idActividad=$EjeEntregableGeneral
    order BY Nombre asc";
        $resultado11 = $catalogo->obtenerLista($consultaperiodo11);
        echo '<option value = "">Seleccione</option>';
        while ($row = mysqli_fetch_array($resultado11)) {
...
{   if ($_GET['EjeEntregableParticular']!="0") {$EjeEntregableParticular=$_GET['EjeEntregableParticular'];
    $consultaperiodo11 = "SELECT IdEntregable,Nombre FROM c_entregable 
    WHERE idActividad=$EjeEntregableParticular
    order BY Nombre asc";
        $resultado11 = $catalogo->obtenerLista($consultaperiodo11);
        echo '<option value = "">Seleccione</option>';
        while ($row = mysqli_fetch_array($resultado11)) {
...
{   if ($_GET['EjeEntregableSubActividad']!="0") {$EjeEntregableSubActividad=$_GET['EjeEntregableSubActividad'];
    $consultaperiodo11 = "SELECT IdEntregable,Nombre FROM c_entregable 
    WHERE idActividad=$EjeEntregableSubActividad
    order BY Nombre asc";
        $resultado11 = $catalogo->obtenerLista($consultaperiodo11);
        echo '<option value = "">Seleccione</option>';
        while ($row = mysqli_fetch_array($resultado11)) {
