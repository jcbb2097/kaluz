        ...
        <form class="form-horizontal" id="formArchivo" name="formArchivo">
            <div class="form-group form-group-sm visible_componentes" >
                ...
                <div class="col-md-2 col-sm-2 col-xs-2">
                    <select id="ano" class="form-control" name="ano" onchange="">
                        ...
                        <?php
                        $Ano = "SELECT p.Id_Periodo,p.Periodo FROM c_periodo as p WHERE p.CPE_ESTATUS=1 and p.Periodo>2019 ORDER BY p.Periodo DESC";
            ...
            <div class="form-group form-group-sm ">
                <div class="col-md-2 col-sm-2 col-xs-2  control-label">
                  <span>  Añadir link a archivo <input type="checkbox" id="checkbox_pdf" onclick="link()" <?php if ($editar == true && $PDF == "link") echo "checked"; ?> /></span>
                </div>
                <div class="col-md-4 col-sm-4 col-xs-4">
                ...
                <div class="col-md-4 col-sm-4 col-xs-4">
                    <select id="cate" class="form-control" name="cate" onchange="Sub_Categorias();Actividad_global();">
                        ...
                        if ($editar == true) {
                            ...
                            $consultagiro = "SELECT ce.idCategoria,ce.descCategoria FROM c_categoriasdeejes ce LEFT JOIN k_categoriasdeejes_anios kca ON kca.idCategoria=ce.idCategoria
                            LEFT JOIN c_periodo p on p.Periodo=kca.Anio WHERE ce.nivelCategoria=1 AND p.Id_Periodo=$periodo2 AND ce.idEje=$Eje  AND kca.ACME = 1 ORDER BY ce.orden"; ///AND kca.Visible=1 se quito el 14 07 2022 para que se vean todas y no cause conflicto con asuntos
                            $resultado = $catalogo->obtenerLista($consultagiro);
                            echo '<option value="">Seleccione</option>';
                            while ($row = mysqli_fetch_array($resultado)) {
                                $s = '';
                ...
                <div class="col-md-4 col-sm-4 col-xs-4">
                    <select id="subcate" class="form-control" name="subcate" onchange="Actividad_global();">
                        ...
                        if ($editar == true) {
                            ...
                            $consultagiro = "SELECT ce.idCategoria,ce.descCategoria FROM c_categoriasdeejes ce LEFT JOIN k_categoriasdeejes_anios kca ON kca.idCategoria=ce.idCategoria
                            LEFT JOIN c_periodo p on p.Periodo=kca.Anio WHERE ce.idCategoriaPadre=$Categoria and  kca.Visible=1 and p.Id_Periodo=$periodo2 ORDER BY ce.orden ";
                            $resultado = $catalogo->obtenerLista($consultagiro);
                    ...
                    <select id="ActvGlobal" class="form-control" name="ActvGlobal" onchange="Actividad_general();checklist();">
                        ...
                        if ($editar == true) {
                            ...
                            WHERE a.IdEje=$Eje AND a.IdNivelActividad=1 AND IdTipoActividad=$ACME $where AND a.Periodo=$periodo2 ORDER BY a.Orden";*/
                             $consultaperiodo6 = "SELECT a.IdActividad, CONCAT(a.Numeracion,a.Nombre) actividad FROM c_actividad a LEFT JOIN k_actividad_anios ka ON ka.IdActividad=a.IdActividad
                                LEFT JOIN c_periodo p on p.Periodo=ka.Anio
                            WHERE a.IdEje=$Eje AND a.IdNivelActividad=1 AND IdTipoActividad=$ACME $where AND p.Id_Periodo=$periodo2 AND ka.Visible=1 ORDER BY a.Orden";
                ...
                <div class="col-md-4 col-sm-4 col-xs-4">
                    <select id="ActvGeneral" class="form-control" name="ActvGeneral" onchange="checklist();">
                        ...
                        WHERE a.IdActividadSuperior=$Actividad_global ORDER BY a.Orden";
                            /*  echo $consultaperiodo6; */
                            $consultaperiodo6 = "SELECT a.IdActividad,CONCAT(a.Numeracion,a.Nombre) actividad FROM c_actividad as a LEFT JOIN k_actividad_anios ka ON ka.IdActividad=a.IdActividad
     LEFT JOIN c_periodo p on p.Periodo=ka.Anio WHERE a.IdActividadSuperior=$Actividad_global AND ka.Visible=1 and p.Id_Periodo=$periodo2 ORDER BY a.Orden";
                            $resultado6 = $catalogo->obtenerLista($consultaperiodo6);
                            echo '<option value = "">Seleccione</option>';
                            while ($row = mysqli_fetch_array($resultado6)) {
                                $s = '';
