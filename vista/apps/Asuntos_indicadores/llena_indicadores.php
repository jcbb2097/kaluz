...
$catalogo = new Catalogo();
  ...
  if($estatus < 3 )
	...
	$num_archivos_ent = 0;
	$resultado= $catalogo->obtenerLista("SELECT count(doc.id_documento) AS total FROM 	c_documento doc	WHERE doc.Asunto = $idconv and  doc.id_tipo in(9,10,14)");
	while ($row = mysqli_fetch_array($resultado)){
		$num_archivos_ent = $row["total"];
	}
 ...
 <div id="ibloque4" class="ibloque" style="font-size: .8em;"> <?php echo $mensajes; ?> mensaje(s)</div>
 <div id="ibloque5" class="ibloque" onclick="entregables(<?php echo $entregable_especifico; ?>);">I-E: 0% <i class="fas fa-search"></i></div>
  <div id="ibloque6" class="ibloque" onclick="abrirEntregables('1');"></div> <!-- I : 0% <span class="badge badge-ent" style="background-color:red; color:white;">0</span> -->
 <div id="ibloque7" class="ibloque" style="cursor:pointer;" onclick="muestra_archivos(<?php echo $idconv; ?>,2);"> E:  <span class="badge badge-ent" style="background-color:red; color:white;"><?php echo $num_archivos_ent; ?></span> </div>
 ...
 <div id="ibloque10" class="ibloque" style="cursor:pointer;" onclick="muestra_archivos(<?php echo $idconv; ?>,0)">
	 <center>
