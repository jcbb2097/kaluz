...
//echo $cons;
  ...
  while ($rs = mysqli_fetch_array($result)){
	  ...
	  while ($row_act = mysqli_fetch_array($resultado_act)){
		...
		$ultimaAct_texto."<span style='color: #526ee7'>[Av $avance_act%]</span></div> <div class='respuesta'><b style='text-decoration: underline #607D8B;'>".$asuntodesc."</b><br>".$respuestadesc." <div class='fecha'>".$fecha."</div></div> <div class='divResponde' id='responde".$i."'></div></td></tr>";//<td>".$onclick."</td>
...
function entregables(idEntregableEspecifico){
  ...
  $.post("./apps/Planeacion/Insumos.php?accion=guardar&idEntregable="+idEntregableEspecifico,{idUsuario:idUsuario}, function(data) {
    $(".detalleinsumos").html('');
...
function muestra_archivos(asunto){
	var asunto = asunto;
	var idUsuario = $( "#idusr" ).val() ;
	var check = $( "#check" ).val();
  ...
  $.post("apps/Asuntos_indicadores/Archivos.php?&idasunto="+asunto+"&check="+check,{idUsuario:idUsuario}, function(data) {
    $(".detalleinsumos").html('');
    $(".detalleinsumos").html(data);
  });
	//$('iframe#myiframe').attr('src','/<?php echo $sistema; ?>/vista/apps/Planeacion/Insumos.php?accion=guardar&idEntregable='+idEntregableEspecifico);
