...
include_once ('../../Classes/DefinirContenidos.class.php');
...
if (isset($_POST['accion']) && $_POST['accion'] != "") {
    ...
    switch ($_POST['accion']) {
        case 'guardar':
            ...
            $objAC->setDescripcion('Archivo Realizar indice Preliminar del libro '.$parametros['titulo']);
            ...
            $rutaPDF = "resources/aplicaciones/Entregables/Publicaciones/DefinirContenidos/Preliminar/";
                ...
                if($agregoArchivo){
                    ...
                    echo 'Warning: El archivo para el entregable <b>Realizar indice Preliminar (Preliminar) de libro <i>'.$parametros['titulo'].'</i></b> no se generó, intente nuevamente';
                    //return;
                    }
                }

            $rutaPDF = "resources/aplicaciones/Entregables/Publicaciones/DefinirContenidos/Final/";
            $objAC->setDescripcion('Archivo Realizar indice Preliminar Final del libro '.$parametros['titulo']);
            $objAC->setId_tipo('10');
            $agregoArchivo = false;
                if (isset($_FILES[1])) {
                    $agregoArchivo = true;
            ...
            if($agregoArchivo){
                ...
                echo 'Warning: El archivo para el entregable <b>Realizar indice Preliminar (Final) de libro <i>'.$parametros['titulo'].'</i></b> no se generó, intente nuevamente';
                //return;
                }
            }
            ...
            $obj->setFechaEnvio($parametros['fechaEnvio']);
            $obj->setFechaEntrega($parametros['fechaEntrega']);
            $obj->setUsuarioUltimaModificiacion($_POST['usuario']);
            $obj->setUsuarioCreacion($_POST['usuario']);
            $obj->setPantalla('Controller_definirContenidos');
            $exito=true;
            if ($IdArchPreliminar != "" || $IdArchFinal != "") {
                ...
                if($objE->agregarEntregableEspecifico()){
                 ...
                 /*    if ($IdArchPreliminar != "") {
                        $EnVer->setIdEntregableEspecifico($IdEntregableEspecif);
                        $EnVer->setIdArchivoPreliminar($IdArchPreliminar);
                        $EnVer->agregarEntregableEspecificoVersion();
                    } */
                    $obj->setIdEntregableIndicePreliminar($IdEntregableEspecif);
                    for($i=0 ; $i<1 ; $i++){
            ...
            if($exito && $obj->agregarDefinirContenidos()){
        ...
        case 'editar':
            ...
            if($agregoArchivo){
                ...
                if($IdArchPreliminar != "" && $objAC->editaracuerdo()){
                    ...
                    echo 'Error: El archivo para el entregable <b>Realizar indice Preliminar (Preliminar) de libro<i>'.$parametros['titulo'].'</i></b> no se generó, intente nuevamente';
                      return;
                    }
                }
        ...
        $agregoArchivo = false;
            ...
            if($agregoArchivo){
                ...
                if($IdArchFinal != "" && $objAC->editaracuerdo()){
                    echo "<br>El archivo del entregable <b><i>Realizar indice preliminar  (Final)</i></b> ha sido modificado";
                }else{
                    //echo "Error:Entra aqui";
                    if($objAC->nuevoAcuerdo()){
                    $IdArchFinal = $objAC->getId_documento();
                    ...
                    echo 'Error: El archivo para el entregable <b>Realizar indice preliminar (Final) de libro<i>'.$parametros['titulo'].'</i></b> no se generó, intente nuevamente';
                      return;
                    }
                }
            ...
            if($exito && $obj->editarDefinirContenidos()){
