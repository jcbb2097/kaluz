...
class ActividadesMetas {
    ...
    public function obtenerActividadMeta() {
     ...
     //  echo $consultaactivofijo;
        ...
        while ($row = mysqli_fetch_array($result)) {
            ...
            $this->tiene = $row['visible_plan'];
            $this->nombreentregable = $row['nombre_entregable'];
            if ($this->Idscategoria == "") {
    ...
    public function agregarActividadMeta(){
        ...
        //echo "subcategoria";
        $insert = "INSERT INTO c_actividad(Nombre,Anio,Periodo,IdArea,IdEje,IdResponsable,IdTipoActividad,IdNivelActividad,IdActividadSuperior,Fecha,Orden,FechaCreacion,UsuarioCreacion,FechaUltimaModificacion,UsuarioUltimaModificacion,Pantalla, Numeracion, Idcategoria,visible_plan,nombre_entregable)
            VALUES('" . $this->Nombre . "',$numeroperiodo," . $this->Periodo . "," . $this->IdArea. "," . $this->IdEje . "," . $this->IdResponsable . ",".$this->IdTipoActividad."," . $this->IdNivelActividad . "," . $this->IdActividadSuperior . ",NOW()," . $this->Orden . ",NOW(),'" . $this->usuarioCreacion . "',NOW(),'". $this->usuarioUltimaModificacion . "','".$this->pantalla."','".$this->Numeracion."',".$this->Idscategoria.",".$this->tiene.",'".$this->nombreentregable."');";
        }else{
        $catalogo = new Catalogo();
        //echo $this->Idcategoria;
        //echo "categoria";
        $insert = "INSERT INTO c_actividad(Nombre,Anio,Periodo,IdArea,IdEje,IdResponsable,IdTipoActividad,IdNivelActividad,IdActividadSuperior,Fecha,Orden,FechaCreacion,UsuarioCreacion,FechaUltimaModificacion,UsuarioUltimaModificacion,Pantalla, Numeracion, Idcategoria,visible_plan,nombre_entregable)
            VALUES('" . $this->Nombre . "',$numeroperiodo," . $this->Periodo . "," . $this->IdArea. "," . $this->IdEje . "," . $this->IdResponsable . ",".$this->IdTipoActividad."," . $this->IdNivelActividad . "," . $this->IdActividadSuperior . ",NOW()," . $this->Orden . ",NOW(),'" . $this->usuarioCreacion . "',NOW(),'". $this->usuarioUltimaModificacion . "','".$this->pantalla."','".$this->Numeracion."',".$this->Idcategoria.",".$this->tiene.",'".$this->nombreentregable."');";
        }
 ...
 //Para el año visible
    //visible por año
    public function kvisibleActividad()
    {
        //Si existe el año
        $catalogo = new Catalogo();
        $valor="";
        $consultaultimoregistro = "SELECT MAX(ca.IdActividad) AS id FROM c_actividad ca";
        $result = $catalogo->obtenerLista($consultaultimoregistro);
        while ($row = mysqli_fetch_array($result)) {
        $valor = $row['id'];
        }

        $insert = "INSERT INTO k_actividad_anios(IdActividad,Anio,Visible)
            VALUES(" . $valor. "," . $this->aniovisible . "," . $this->visible . ");";
        $this->IdActividad = $catalogo->insertarRegistro($insert);
    ...
    public function kvisibleeditarActividad()
    {
        $catalogo = new Catalogo();
        $existe="";
        $validasiexiste="SELECT ca.IdActividad AS id FROM k_actividad_anios ca WHERE ca.IdActividad=$this->IdActividad AND ca.Anio=$this->aniovisible";
        $result = $catalogo->obtenerLista($validasiexiste);
        while ($row = mysqli_fetch_array($result)) {
        $existe = $row['id'];
        }
        ...
        if ($existe == "") {
        $insert = "INSERT INTO k_actividad_anios(IdActividad,Anio,Visible)
            VALUES(" . $this->IdActividad. "," . $this->aniovisible . "," . $this->visible . ");";
        $this->IdActividad = $catalogo->insertarRegistro($insert);
        //echo "<br><br>$insert<br><br>"; 
        if ($this->IdActividad == 0 || $this->IdActividad == null) {
            return false;
        ...
        $consulta = "UPDATE k_actividad_anios SET Visible = " . $this->visible . " WHERE IdActividad = " . $this->IdActividad . " AND Anio=". $this->aniovisible . ";";
        $query = $catalogo->ejecutaConsultaActualizacion($consulta, 'k_actividad_anios', 'IdActividad = ' . $this->IdActividad.' AND Anio='. $this->aniovisible);
        //echo "<br><br>$consulta<br><br>";
        if ($query == 1) {
            return true;
        }
    ...
    public function eliminaaniosvisibles()
    {
        $catalogo = new Catalogo();
        $consulta = "DELETE FROM k_actividad_anios WHERE IdActividad= " . $this->IdActividad . ";";
        $query = $catalogo->ejecutaConsultaActualizacion($consulta, "k_actividad_anios", "IdActividad = " . $this->IdActividad);
        //echo "<br><br>$consulta<br><br>"; 
        if ($query == 1) {
            return true;
        }
    ...
    function getnombreentregable() {
        return $this->nombreentregable;
    }

    function getaniovisible() {
        return $this->aniovisible;
    }
    ...
    function setaniovisible($aniovisible) {
    ...
    function setnombreentregable($nombreentregable) {
