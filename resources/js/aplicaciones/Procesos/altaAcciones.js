    ...
    // $(".dataTables_filter").hide();
        ...
        fields:{
            ...
            responsable: {
                validators: {
                    notEmpty: {
                        message: 'Ingresa un responsable'
                    }
                }
            }
        }
     ...
     $("#guardar").click(function() { 
        ...
        if ($(form).bootstrapValidator('validate').has('.has-error').length === 0 ) {
                   ...
                   formData.append('IdAccion',$("#IdAccion").val());
                         ...
                         $.ajax({
                       url: 'WEB-INF/Controllers/Aplicaciones/Controler_Acciones.php',
                       type: 'POST',
                       data: formData,
                       cache: false,
                       contentType: false,
...
swal({
  title: '¿Desea eliminar la accion?',
  ...
  if (result.value) { 
        $.post('WEB-INF/Controllers/Aplicaciones/Controler_Acciones.php',{accion:'eliminar',IdAccion:id},function(data){
           if (data.toString().indexOf("Error:") === -1) {
            swal(data,"","success");
        }else{
            swal(data,"","warning");
        }
        cambiarContenidosConMensaje('#contenidoProcesos','Procesos/listaAcciones.php',data);
        });
  }
});
}
